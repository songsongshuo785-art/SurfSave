...
class VideoServiceSuperX(
                ...
                manifest.variants.mapNotNullTo(formats) { variant ->
                    ...
                    if (width > 0 && height > 0) {
                        ...
                        VideoFormatEntity(
                            ...
                            duration = duration,
                            fileSizeApproximate = estimateSizeBytes(duration, combinedBitrate)
                        )
                    } else {
    ...
    private fun parseMpdManifest(
        ...
        val formats = allVideoRepresentations.mapNotNull { rep ->
            ...
            VideoFormatEntity(
                ...
                fileSizeApproximate = estimateSizeBytes(durationInMillis, rep.bandwidth)
            )
        }
    ...
    private fun estimateSizeBytes(durationMs: Long, bitrateBitsPerSecond: Long): Long {
        if (durationMs <= 0 || bitrateBitsPerSecond <= 0) {
            return 0L
        }

        return ((durationMs / 1000.0) * bitrateBitsPerSecond / 8.0).toLong()
    }
