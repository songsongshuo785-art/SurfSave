...
interface BookmarksListener {
    ...
    fun onBookmarkMoreClicked(view: View, bookmarkItem: PageInfo, position: Int)
...
class BookmarksAdapter(
    ...
    class BookmarkViewHolder(val binding: ItemBookmarkBinding) :
        ...
        fun bind(bookmarkItem: PageInfo, bookmarksListener: BookmarksListener) {
            with(binding) {
                ...
                this.ivMore.visibility = View.VISIBLE
                this.ivMore.setOnClickListener {
                    if (currentPosition != RecyclerView.NO_POSITION) {
                        bookmarksListener.onBookmarkMoreClicked(it, bookmarkItem, currentPosition)
                    }
                }
    ...
    fun setData(bookmarksItems: MutableList<PageInfo>) {
        val newItems = bookmarksItems.toMutableList()
        dispatchListDiff(
            oldItems = this.bookmarksItems,
            newItems = newItems,
            areItemsTheSame = { oldItem, newItem -> oldItem.id == newItem.id }
        ) {
            this.bookmarksItems = newItems
        }
    }
