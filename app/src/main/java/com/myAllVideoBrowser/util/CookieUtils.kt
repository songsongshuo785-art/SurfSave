...
object CookieUtils {
    ...
    fun deleteTemporaryCookieFile(cookieFile: File?) {
        ...
        val cacheDir = ContextUtils.getApplicationContext().cacheDir
        val isOwnedTempFile = runCatching {
            file.isFile && file.canonicalFile.parentFile == cacheDir.canonicalFile
        }.getOrDefault(false)
        if (isOwnedTempFile && !file.delete()) {
