...
class ProxyService : Service() {
    ...
    private fun handleSystemStartOrRestart() {
        ...
        proxyJob = serviceScope.launch {
            ...
            if (!startForegroundWithCorrectType()) {
                return@launch
            }
    ...
    private fun handleStartOrUpdate(intent: Intent) {
        ...
        val proxyHops = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            intent.getSerializableExtra(EXTRA_PROXY_HOPS, ArrayList::class.java).toProxyHopList()
        } else {
            @Suppress("DEPRECATION")
            intent.getSerializableExtra(EXTRA_PROXY_HOPS).toProxyHopList()
        }
     ...
     */
    private fun startForegroundWithCorrectType(): Boolean {
        ...
        return try {
            ...
            AppLogger.e("Failed to promote ProxyService to foreground: ${e.message}")
            stopSelf()
            false
        }
    }

    private fun Any?.toProxyHopList(): List<ProxyHop> {
        return (this as? Iterable<*>)?.filterIsInstance<ProxyHop>().orEmpty()
    }
