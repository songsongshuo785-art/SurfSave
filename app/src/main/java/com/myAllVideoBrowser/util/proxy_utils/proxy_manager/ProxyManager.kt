...
object ProxyManager {
    ...
    fun startProxyChain(
        ...
        if (!V2Ray.isNativeLoaded()) {
            Log.e(TAG, "V2Ray native library is unavailable; local proxy cannot start.")
            return false
        }
        ...
        try {
            val result = V2Ray.start(xrayJsonConfig)
            if (result == 0L) {
                Log.i(TAG, "V2Ray proxy chain started successfully on port $localPort")
                return true
            } else {
                Log.e(TAG, "V2Ray.start returned a non-zero error code: $result")
                return false
            }
        } catch (e: Throwable) {
    ...
    fun stopLocalProxy() {
        if (!V2Ray.isNativeLoaded()) return
    ...
    fun isProxyRunning(): Boolean {
        if (!V2Ray.isNativeLoaded()) return false
        return try {
            V2Ray.isRunning()
        } catch (e: Throwable) {
