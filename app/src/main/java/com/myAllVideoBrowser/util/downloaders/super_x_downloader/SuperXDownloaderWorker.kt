...
class SuperXDownloaderWorker(appContext: Context, workerParams: WorkerParameters) :
    ...
    private val activeControllers = CopyOnWriteArrayList<FileBasedDownloadController>()
    ...
    private fun startDownload(task: VideoTaskItem, headersRaw: Map<String, String>) {
        ...
        downloadTaskLogger.info(taskId, "SuperX download started: ${task.fileName}")
            ...
            try {
                ...
                when {
                    ...
                    controller.isPauseRequested() || e is CancellationException -> {
                        ...
                        AppLogger.e("MPD: Download failed for task $taskId: ${e.message}", e)
                        finishWork(task.also {
                            it.taskState = VideoTaskState.ERROR
            ...
            try {
                ...
                when {
                    ...
                    // Handle other errors.
                        ...
                        AppLogger.e("HLS: Download failed for task $taskId: ${e.message}", e)
                        finishWork(task.also {
                            it.taskState = VideoTaskState.ERROR
    ...
    private fun handleTaskCompletion(item: VideoTaskItem) {
        ...
        when (item.taskState) {
            ...
            VideoTaskState.SUCCESS -> {
                ...
                downloadTaskLogger.error(item.mId, "SuperX download failed: ${item.errorMessage ?: "Unknown error"}")
                saveProgress(
                    item.mId,
                    finalProgress,
                    item.taskState,
        ...
        return headers.toMutableMap().also { fixedHeaders ->
            fixedHeaders["Cookie"]?.let {
                try {
                    ...
                    AppLogger.e(
                        "SuperX: Failed to decode Base64 Cookie header, length=${it.length}",
                        e
                    )
                }
            }
